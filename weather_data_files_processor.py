...
def generate_report(parsed_list, info_type):
    ...
    if info_type == '-e':
        ...
        print("\n{} {}".format(calendar.month_name[int(parsed_list[0].date.split("-")[1])], parsed_list[0].date.split("-")[0]))
        for value in parsed_list:
            ...
            for mt in range(value.lowest_temprature):
                lowest_temprature_bar += '+'
            if info_type == '-c':
                print("{} {} {}C".format(str(value.date.split('-')[2]).zfill(2), colored(max_temprature_bar, 'red'), value.max_temprature))
                print("{} {} {}C".format(str(value.date.split('-')[2]).zfill(2), colored(lowest_temprature_bar, 'blue'), value.lowest_temprature))
                max_temprature_bar = ''
                lowest_temprature_bar = ''
            if info_type == '-d':
                print("{} {}{} {}C - {}C".format(str(value.date.split('-')[2]).zfill(2), colored(lowest_temprature_bar, 'blue'), colored(max_temprature_bar, 'red'), value.lowest_temprature, value.max_temprature))
                max_temprature_bar = ''
                lowest_temprature_bar = ''
